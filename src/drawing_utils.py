...
def text_with_background(image, text, position, text_color=colors.WHITE, background_color=colors.GRAY, background_opacity=1.0, padding_x=5, padding_y=5):
	...
	x, y = position
	overlay = image.copy()
	cv.rectangle(overlay, (x, y), (x+text_width+2*padding_x, y+text_height+2*padding_y), background_color, -1)
	new_image = cv.addWeighted(overlay, background_opacity, image, 1-background_opacity, 0)
	cv.putText(new_image, text, (x+padding_x, y+padding_y+text_height), cv.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 1)
	image = new_image
	return image
